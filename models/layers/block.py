...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, attn_mask: Optional[Tensor] = None) -> Tensor:
        def attn_residual_func(x: Tensor, attn_mask: Optional[Tensor] = None) -> Tensor:
            x2, attn_weights = self.attn(self.norm1(x), attn_mask=attn_mask)
            if self.training and self.sample_drop_ratio > 0.0:
                return self.drop_path1(self.ls1(x2)) + x, attn_weights
            else:
                return self.ls1(x2) + x, attn_weights
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            ...
            # FIXME: need to return attn_weights
            ...
            x, attn_weights = attn_residual_func(x, attn_mask=attn_mask)
            x = x + self.drop_path2(ffn_residual_func(x)) if self.training and self.sample_drop_ratio > 0.0 else x + ffn_residual_func(x)
...
class NestedTensorBlock(Block):
    ...
    def forward(self, x_or_x_list, attn_mask: Optional[Tensor] = None):
        if isinstance(x_or_x_list, Tensor):
            return super().forward(x_or_x_list, attn_mask=attn_mask)
        elif isinstance(x_or_x_list, list):
            if not XFORMERS_AVAILABLE:
                raise AssertionError("xFormers is required for using nested tensors")
            return self.forward_nested(x_or_x_list, attn_mask=attn_mask)
        else:
