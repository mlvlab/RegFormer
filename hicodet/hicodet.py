...
class HICODet(ImageDataset):
    ...
    def __init__(self, root: str, anno_file: str,
        ...
        # Load annotations
        self.args = args
        self._load_annotation_and_metadata(anno, eval_mode=args.eval_mode if 'test' in root else 'all')
    ...
    def __getitem__(self, i: int) -> tuple:
        ...
        return self._transforms(
            ...
            ),self._filenames[intra_idx]
    
    def get_few_shot_sample(self, intra_idx: int) -> tuple:
        return self._transforms(
            self.load_image(os.path.join(self._root, self._filenames[intra_idx])), 
            self._anno[intra_idx]
            ),self._filenames[intra_idx]
    ...
    def interaction_to_verb(self) -> List[list]:
        ...
        inter_to_verb = []
        for i, corr in enumerate(self._class_corr):
    ...
    def distance_stat_calculation(self, anno, size):

        assert len(anno) == len(size), "Number of annotations and sizes must match"
        print("calculating distance statistics...")
        max_distance = self.args.max_distance
        distance_interval = self.args.distance_interval
        num_intervals = int(max_distance / distance_interval) + 1  # +1 for the final interval (>max_distance)
        
        # Initialize counts for each interval
        if self.args.class_wise_ap:
            distance_counts = [[0] * self.num_interation_cls for _ in range(num_intervals)]
        else:
            distance_counts = [0] * num_intervals
        
        for idx, annotation in enumerate(anno):
            if 'boxes_h' in annotation and 'boxes_o' in annotation:
                boxes_h = np.array(annotation['boxes_h'])
                boxes_o = np.array(annotation['boxes_o'])
                
                if len(boxes_h) > 0 and len(boxes_o) > 0:
                    # Get image size for normalization
                    w, h = size[idx]
                    
                    # Normalize coordinates
                    norm_boxes_h = boxes_h / np.array([w, h, w, h])
                    norm_boxes_o = boxes_o / np.array([w, h, w, h])
                    ...
                    distances = np.sqrt(np.sum((center_boxes_h - center_boxes_o) ** 2, axis=1))
                    
                    # Count distances in each interval
                    for ii,distance in enumerate(distances):
                        interval_idx = min(int(distance / distance_interval), num_intervals - 1)
                        if self.args.class_wise_ap:
                            distance_counts[interval_idx][annotation['hoi'][ii]] += 1
                        else:
                            distance_counts[interval_idx] += 1
        
        # print("distance statistics calculated")
        # print(distance_counts)
        return distance_counts

    def _load_annotation_and_metadata(self, f: dict, eval_mode: str) -> None:
        """
        Arguments:
        ...
        if eval_mode == 'single':
            for id_, anno in enumerate(f['annotation']):
                hoi_list = anno['hoi']
                if len(hoi_list) != len(set(hoi_list)) and id_ in idx:
                    idx.remove(id_)
            print(f"Number of images after removing duplicate annotations for single eval mode: {len(idx)}")
        elif eval_mode == 'multi':
            for id_, anno in enumerate(f['annotation']):
                hoi_list = anno['hoi']
                if len(hoi_list) == len(set(hoi_list)) and id_ in idx:
                    idx.remove(id_)
            print(f"Number of images after removing unique annotations for multi eval mode: {len(idx)}")
        ...
        self.distance_statistics = (
            self.distance_stat_calculation(f['annotation'], f['size'])
            if getattr(self.args, 'eval_distance', False)
            else None
        )
