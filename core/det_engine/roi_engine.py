...
class CustomisedDLE(DistributedLearningEngine):
    ...
    def test_hico(self, dataloader, datasetname='hicodet', detr_backbone='R50'):
        ...
        for batch in tqdm(dataloader):
            ...
            continue
        # pdb.set_trace() 
        detector_name = self.args.pretrained.rsplit('/',1)[-1].split('.')[0]
        output_dir = os.path.join('data', f'{datasetname}_pkl_files')
        os.makedirs(output_dir, exist_ok=True)
        output_path = os.path.join(output_dir, f'{datasetname}_test_bbox_{detr_backbone}_{detector_name}.p')
        print(f"save detector boxes to {output_path}")
